...
class SketchApp:
    ...
    def initialize_all(self):
        self.input_prompt = sketch_first_prompt.format(concept=self.target_concept, examples=gt_example)
    ...
    def submit_sketch(self):
        ...
        # Save final LLM output as JSON in concept-specific folder
        human_path = f"results/human/{self.target_concept}"
        os.makedirs(human_path, exist_ok=True)
        output_data = {
            "session_id": self.session_id,
            "timestamp": datetime.now().strftime("%Y-%m-%d_%H-%M-%S"),
            "llm_output": self.assitant_history
        }
        with open(f"{human_path}/{self.session_id}.json", "w") as f:
            json.dump(output_data, f, indent=2)
        print(f"Final LLM output saved to [{human_path}/{self.session_id}.json]")
